...
const projectsData: Project[] = [
  {
    ...
    businessImpact:
      'Enabling law firms to handle more clients through intelligent workflow automation',
    longDescription:
      'Software Engineer at Glade AI, working on automation systems that eliminate repetitive administrative tasks for law firms across bankruptcy, immigration, and other practice areas.',
    status: 'ACTIVE',
...
• **Update vs create strategy**: Updates existing records rather than creating duplicates`,
  ...
  },
  {
    ...
    description:
      'Prompt engineering experiments for automated e-commerce title generation at scale',
...
export const getProjectsBySkill = (skillName: string): Project[] => {
  return projects.filter(project => project.skills.some(skill => skill.name === skillName))
}

export const getProjectsByCategory = (category: SkillCategory): Project[] => {
  return projects.filter(project => project.skills.some(skill => skill.category === category))
}
