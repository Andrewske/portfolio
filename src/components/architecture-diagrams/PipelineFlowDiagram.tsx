...
export function PipelineFlowDiagram({
  ...
  useEffect(() => {
    ...
    const links = linkGroup.selectAll('g').data(scaledData.links).enter().append('g')
    ...
    const linkLabels = links.append('g').attr('transform', d => {
      const sourceNode = scaledData.nodes.find(n => n.id === d.source)
      const targetNode = scaledData.nodes.find(n => n.id === d.target)
      if (!sourceNode || !targetNode) return ''
    ...
    nodes.each(function (d) {
      ...
      if (d.type === 'database') {
        ...
        node
          ...
          .attr(
            'd',
            `M 0 ${-nodeHeight / 2} L ${nodeWidth / 2} 0 L 0 ${nodeHeight / 2} L ${-nodeWidth / 2} 0 Z`,
          )
          .attr('fill', `url(#gradient-${d.type})`)
          .attr('stroke', nodeColors[d.type])
          .attr('stroke-width', 2)
      ...
      nodes
        .on('mouseenter', function (_event, d) {
          ...
          linkPaths
            ...
            .attr('stroke-width', link => (link.source === d.id || link.target === d.id ? 3 : 2))

          // Highlight connected nodes
          nodes.attr('opacity', node => {
            const isConnected = scaledData.links.some(
              link =>
    ...
    if (scaledData.title || scaledData.description) {
      const titleGroup = svg.append('g').attr('transform', `translate(30, 40)`)
