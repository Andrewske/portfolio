...
const TechStackVisualization = () => {
  ...
  const mobileNodes = useMemo(() => {
    ...
    const getSkillPriority = (skill: any) => {
      ...
      if (['TypeScript', 'Python', 'JavaScript'].includes(skill.name))
        return 900 + (skill.experience || 0)
      // Modern frameworks get good priority
      if (['React', 'Next.js', 'PostgreSQL'].includes(skill.name))
        return 800 + (skill.experience || 0)
  ...
  useEffect(() => {
    ...
    const _centerY = (height - margin.top - margin.bottom) / 2
    ...
    labelBg
      ...
      .attr('y', (d: any) => {
        const yOffset = isMobile ? 14 : d.type === 'project' ? 30 : 20
    ...
    node
      ...
      .on('mouseenter', (_event, d) => {
        if (!selectedNode) {
          // Only show hover if no node is selected
          highlight(d.id)
        }
      })
      .on('mouseleave', (_event, _dd) => {
        if (!selectedNode) {
          // Only clear hover if no node is selected
          highlight(null)
        }
      })
    ...
    simulation.on('tick', () => {
      ...
      nodes.forEach((d: any) => {
        ...
        d.x = Math.max(
          horizontalPadding,
          Math.min(width - margin.left - margin.right - horizontalPadding, d.x),
        )
        d.y = Math.max(
          topPadding,
          Math.min(height - margin.top - margin.bottom - bottomPadding, d.y),
        )
      })
  ...
  const getNodeInfo = (nodeId: string) => {
    ...
    if (node.type === 'project') {
      ...
      const connectedProjects = isMobile
        ? []
        : currentLinks
            .filter(
              link =>
                ...
                <>
                      ...
                      style={{
                        backgroundColor:
                          selectedNodeInfo.type === 'project'
                            ? '#06b6d4'
                            : selectedNodeInfo.category === 'Languages'
                              ? '#a855f7'
                              : selectedNodeInfo.category === 'AI/ML'
                                ? '#22d3ee'
                                : selectedNodeInfo.category === 'Frontend'
                                  ? '#3b82f6'
                                  : selectedNodeInfo.category === 'Backend'
                                    ? '#eab308'
                                    : selectedNodeInfo.category === 'Data & Analytics'
                                      ? '#10b981'
                                      : selectedNodeInfo.category === 'APIs & Integrations'
                                        ? '#f97316'
                                        : selectedNodeInfo.category === 'Infrastructure'
                                          ? '#ef4444'
                                          : '#6b7280',
                      }}
                    />
                  ...
                  {selectedNodeInfo.type === 'skill' && (
                    <div className="mb-4">
                      ...
                      <div className="text-cyan-400 font-mono">
                        {selectedNodeInfo.experience} years
                      </div>
                    </div>
                  )}
                  ...
                  <div className="mb-4">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      {selectedNodeInfo.type === 'project' &&
                        (selectedNodeInfo as any).allSkills?.length > 0 && (
                          <button
                    ...
                    <div className="space-y-1">
                      {selectedNodeInfo.type === 'project' ? (
                        showAllSkills ? (
                          ...
                          <div className="space-y-3 max-h-40 sm:max-h-60 overflow-y-auto">
                            {(selectedNodeInfo as any).allSkills?.map(
                              ([category, categorySkills]: [string, any[]]) => (
                                <div key={category} className="space-y-1">
                                  <div
                                    className={`text-xs font-semibold ${getCategoryColor(category as any)}`}
                                  >
                                    {category}
                                  </div>
                          ...
                          (selectedNodeInfo as any).connectedSkills?.map(
                      ...
                      containerWidth < 640 ? (
                        <div className="text-sm text-gray-300 font-mono">
                          Category: {selectedNodeInfo.category}
                          <br />
                          Experience: {selectedNodeInfo.experience} years
                        </div>
                      ) : (
                        (selectedNodeInfo as any).connectedProjects?.map(
