...
const highlightTypeScript = (code: string): React.ReactNode[] => {
  ...
  return lines.map((line, lineIndex) => {
    ...
    // Built-ins
    const _builtins = ['console', 'new', 'Date']
          ...
          for (const keyword of keywords) {
            if (
              ...
              (remaining[keyword.length] === ' ' ||
                remaining[keyword.length] === '(' ||
                remaining[keyword.length] === ':' ||
                remaining[keyword.length] === undefined)
            ) {
              tokens.push(
          ...
          if (!keywordMatched) {
            ...
            for (const type of types) {
              if (
                ...
                (remaining[type.length] === '<' ||
                  remaining[type.length] === ',' ||
                  remaining[type.length] === '>' ||
                  remaining[type.length] === ' ' ||
                  remaining[type.length] === undefined)
              ) {
                tokens.push(
        ...
        // Keywords
          ...
          if (!keywordMatched) {
            ...
            for (const builtin of builtins) {
              if (
                remaining.startsWith(builtin) &&
                (remaining[builtin.length] === '(' ||
                  remaining[builtin.length] === ' ' ||
                  remaining[builtin.length] === undefined ||
                  /\W/.test(remaining[builtin.length] || ''))
              ) {
                tokens.push(
                  <span key={`${lineIndex}-${index}`} className="text-purple-400">
                    {builtin}
        ...
        // Keywords (case insensitive)
          ...
          if (!keywordMatched) {
            ...
            for (const func of functions) {
              if (
                remaining.toUpperCase().startsWith(func) &&
                (remaining[func.length] === '(' ||
                  remaining[func.length] === ' ' ||
                  remaining[func.length] === undefined ||
                  /\W/.test(remaining[func.length] || ''))
              ) {
                tokens.push(
                  <span key={`${lineIndex}-${index}`} className="text-purple-400">
    ...
    <div className="space-y-4">
      ...
      <div className="border border-gray-800 rounded-lg overflow-hidden">
        ...
        <div className="p-3 sm:p-4 bg-gray-950 text-xs sm:text-sm md:text-base font-mono overflow-x-auto">
          <div className="min-w-max">{highlightedCode}</div>
        </div>
      </div>
