  ...
  return groups
    .map((group, groupIndex) => {
      ...
      if (group.isTerminal) {
        ...
        return <DinoCollapsible key={`group-${groupIndex}`}>{renderedBlocks}</DinoCollapsible>
      }
    ...
    <div className="min-h-screen bg-bg-main text-text-primary font-[family-name:var(--font-ibm-plex)]">
      ...
      <section>
        ...
        <div className="max-w-4xl mx-auto px-2">
          ...
          <div
            id="tldr"
            className="space-y-6 mb-12 sm:mb-16 bg-amber-300/10 p-4 rounded-lg scroll-mt-16"
          >
            <h2 className="text-xl sm:text-3xl font-mono font-bold text-[#7ee787]">TL;DR</h2>

            <p className="text-[#8b949e] text-base leading-relaxed">{tldr.summary}</p>
              ...
              {phases.map(phase => (
                <div key={phase.id} className="flex gap-3">
                  ...
                  <p className="text-[#c9d1d9] text-base leading-relaxed">
                    <span className="font-mono font-semibold text-[#7ee787]">
          ...
          <div className="space-y-8">
            {phases.map(phase => (
              <section key={phase.id} id={phase.id} className="space-y-6 scroll-mt-16">
                ...
                <div className="space-y-4">{renderBlockGroups(phase.blocks, noDinos)}</div>
              </section>
            ))}
          </div>
