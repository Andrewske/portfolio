  ...
  return (
    ...
    >
      <div className="text-text-primary font-semibold">
        Finding (<span className={styles.text}>{severityLabel}</span>): {title}
        {confidenceStr}
      </div>
      <div className="space-y-3 text-text-body leading-relaxed">{renderBlocks(blocks)}</div>
    </div>
  )
}
